    ...
    computeObjFunc <- function(weights, rhos) {
      ...
      if (forceApproxK) {
        ...
        if (class(res) %in% "try-error") {
          message("Since K is not positive semi-definite during the optimization of weights, it is approximated with the nearest nearest positive definite matrix.")
  ...
  if (lz >= 2) {
    ...
    if (forceApproxK) {
      ...
      if (class(EMM.cpp.res) %in% "try-error") {
        message("Since K is not positive semi-definite after the optimization of weights, it is approximated with the nearest nearest positive definite matrix.")
...
#' Calculate Standard Errors for Variance Parameters
#'
#' @description
...
#'
#' @param y A \eqn{n \times 1} vector. A vector of phenotypic values should
#' be used. NA is allowed. The same phenotypic vector used in
#' \code{EMM.cpp} or \code{EM3.cpp} should be assigned.
#'
#' @param MM.res A list returned from the \code{EMM.cpp} or \code{EM3.cpp}
...
#' \code{weights}.
#'
#' @param ZETA A list of variance matrices and their design matrices of
#' random effects.
#' For example,
...
#' \code{"Z"} and \code{"K"}.
#'
#' @param X0 A \eqn{n \times p} matrix. The same fixed-effect design matrix
...
#'
#' @param REML If \code{TRUE}, the expected Fisher information matrix for
...
#'
#' @param check.Vinv If \code{TRUE}, the reconstructed inverse
#' variance-covariance matrix is compared with \code{MM.res$Vinv} when
#' \code{Vinv} is available in \code{MM.res}.
#'
#' @param Vinv.tol A positive numeric value specifying the tolerance used
#' when comparing the reconstructed inverse variance-covariance matrix with
#' \code{MM.res$Vinv}.
#'
#' @param suppressWarning If \code{TRUE}, warning messages generated by this
...
#' }
#'
#' @example R/examples/calcParamSE_example.R
#'
#' @export
calcParamSE <- function(
    y,
    MM.res,
    ZETA,
    X0 = NULL,
  ...
  requiredResults <- c("Vu", "Ve")
  missingResults <- requiredResults[
    !requiredResults %in% names(MM.res)
  ]

  if (length(missingResults) > 0L) {
    stop(
      paste0(
        "The following elements are missing from MM.res: ",
        paste(missingResults, collapse = ", "),
        "."
      )
    )
  }
  ...
  if (!is.logical(REML) ||
      length(REML) != 1L ||
      is.na(REML)) {
    stop("REML should be TRUE or FALSE.")
  }

  if (!is.logical(check.Vinv) ||
      length(check.Vinv) != 1L ||
      is.na(check.Vinv)) {
    stop("check.Vinv should be TRUE or FALSE.")
  }

  if (!is.numeric(Vinv.tol) ||
      length(Vinv.tol) != 1L ||
      !is.finite(Vinv.tol) ||
      Vinv.tol <= 0) {
    stop("Vinv.tol should be a positive finite numeric value.")
  }

  if (!is.logical(suppressWarning) ||
      length(suppressWarning) != 1L ||
      is.na(suppressWarning)) {
    stop("suppressWarning should be TRUE or FALSE.")
  }

  y <- as.matrix(y)

  if (ncol(y) != 1L) {
    stop("y should be an n x 1 vector or matrix.")
  }

  nOriginal <- nrow(y)
  not.NA <- which(!is.na(y[, 1]))

  if (length(not.NA) == 0L) {
    stop("No non-missing phenotypic observations were found.")
  }

  n <- length(not.NA)
  nKernels <- length(ZETA)

  if (nKernels < 1L) {
    stop("ZETA should contain at least one random effect.")
  }

  for (i in seq_len(nKernels)) {
    ...
    if (is.null(ZETA[[i]]$K)) {
      stop(
        paste0(
          "ZETA[[",
          i,
          "]] should contain an element named 'K'."
        )
      )
    }

    Ki <- as.matrix(ZETA[[i]]$K)
    Zi <- ZETA[[i]]$Z
    ...
    if (nrow(Zi) != nOriginal) {
      stop(
        paste0(
          "The number of rows of ZETA[[",
          i,
          "]]$Z should be equal to the length of y."
        )
      )
    }
  ...
  if (!is.numeric(Vu) ||
      ...
      !is.finite(Vu) ||
      Vu < 0) {
    stop("MM.res$Vu should be a non-negative finite numeric value.")
  }

  if (Vu == 0 && !suppressWarning) {
    warning(
      "MM.res$Vu is zero. The corresponding variance component is ",
      "on the boundary of the parameter space, and the normal ",
      "approximation for its standard error should be interpreted ",
      "with caution."
    )
  }
  ...
  if (!is.numeric(Ve) ||
      ...
      !is.finite(Ve) ||
      Ve <= 0) {
    stop("MM.res$Ve should be a positive finite numeric value.")
  }
  ...
  if (nKernels == 1L) {
    ...
    if (is.null(MM.res$weights)) {
      stop(
        "MM.res$weights is required when ZETA contains more than ",
        "one kernel. Please supply a result returned from EM3.cpp."
      )
    }

    weights <- as.numeric(MM.res$weights)

    if (length(weights) != nKernels) {
      stop(
        "The number of elements in MM.res$weights should be equal ",
        "to the number of random effects in ZETA."
      )
    }

    if (any(!is.finite(weights))) {
      stop("Non-finite values were found in MM.res$weights.")
    }

    if (any(weights < 0)) {
      stop("All elements of MM.res$weights should be non-negative.")
    }

    if (!is.finite(sum(weights)) ||
        sum(weights) <= 0) {
      stop("The sum of MM.res$weights should be positive and finite.")
    }

    if (any(weights == 0) && !suppressWarning) {
      warning(
        "Zero weight(s) were detected. The corresponding variance ",
        "component estimate is on the boundary of the parameter space, ",
        "and its standard error should be interpreted with caution."
      )
    }

    weights <- weights / sum(weights)
    variances <- Vu * weights
    model <- "multi-kernel"
  }
  ...
  if (any(!is.finite(variances)) ||
      any(variances < 0)) {
    stop(
      "All kernel-specific variance estimates should be ",
      "non-negative and finite."
    )
  }

  kernelNames <- names(ZETA)

  if (is.null(kernelNames) ||
      anyNA(kernelNames) ||
      any(kernelNames == "") ||
      anyDuplicated(kernelNames)) {
    kernelNames <- paste0("K", seq_len(nKernels))
  }

  names(variances) <- kernelNames
  ...
  if (is.null(X0)) {
    ...
    if (nrow(X0) != nOriginal) {
      stop(
        "The number of rows in X0 should be equal to the length of y."
      )
    }

    if (anyNA(X0)) {
      stop("NA is not allowed in X0.")
    }
  ...
  if (qr(X)$rank < ncol(X)) {
    stop("X0 is not of full column rank.")
  }

  ZList <- vector("list", nKernels)
  KList <- vector("list", nKernels)

  names(ZList) <- kernelNames
  names(KList) <- kernelNames
  ...
  for (i in seq_len(nKernels)) {
    ...
    KList[[i]] <- as.matrix(ZETA[[i]]$K)
  }

  AList <- vector("list", nKernels)
  names(AList) <- kernelNames
  ...
  if (is.null(Vchol)) {
    stop(
      "The reconstructed variance-covariance matrix V is not ",
      "positive definite."
    )
  }
  ...
  if (check.Vinv &&
    ...
    VinvMM <- as.matrix(MM.res$Vinv)

    if (all(dim(VinvMM) == c(n, n))) {
      ...
      if (is.finite(VinvDifference) &&
          VinvDifference > Vinv.tol &&
          !suppressWarning) {
        warning(
          paste0(
            "The reconstructed inverse variance-covariance matrix ",
            "differs from MM.res$Vinv (maximum absolute difference = ",
            signif(VinvDifference, 5),
            "). Please confirm that y, X0, ZETA, and REML are the ",
            "same as those used for model fitting."
          )
        )
      }
      ...
      if (!suppressWarning) {
        warning(
          "MM.res$Vinv was not compared with the reconstructed ",
          "matrix because their dimensions were different."
        )
      }
    }
  }
  ...
  if (REML) {
    ...
    if (is.null(XVinvXchol)) {
      stop(
        "X'V^{-1}X is not positive definite. ",
        "Please check X0 and the fitted model."
      )
    }

    XVinvXinv <- chol2inv(XVinvXchol)
  ...
  VDeriv <- list()
  parameterNames <- character(0)
  parameterTypes <- character(0)
  estimates <- numeric(0)
  ...
  for (i in seq_len(nKernels)) {
    ...
    VDeriv[[varianceName]] <- AList[[i]]
    parameterNames <- c(parameterNames, varianceName)
    parameterTypes <- c(parameterTypes, "Variance")
    estimates <- c(estimates, variances[i])
  }

  VDeriv[["Ve"]] <- diag(n)
  parameterNames <- c(parameterNames, "Ve")
  parameterTypes <- c(parameterTypes, "Residual variance")
  estimates <- c(estimates, Ve)

  names(estimates) <- parameterNames

  nParams <- length(VDeriv)

  InfoVDeriv <- vector(
    mode = "list",
    length = nParams
  )
  names(InfoVDeriv) <- parameterNames

  for (a in seq_len(nParams)) {
    InfoVDeriv[[a]] <-
      InfoBase %*%
      VDeriv[[a]]
  }
  ...
  for (a in seq_len(nParams)) {
    ...
    for (b in a:nParams) {
      ...
      Fisher[a, b] <- information
      Fisher[b, a] <- information
    }
  }
  ...
  if (min(fisherEigen) <= fisherTol) {
    stop(
      paste0(
        "The expected Fisher information matrix is singular or ",
        "nearly singular (minimum eigenvalue = ",
        signif(min(fisherEigen), 5),
        "). Variance parameters may not be separately identifiable."
      )
    )
  }
  ...
  if (is.null(FisherChol)) {
    stop(
      "The expected Fisher information matrix could not be inverted."
    )
  }

  FisherInv <- chol2inv(FisherChol)
  FisherInv <- (FisherInv + t(FisherInv)) / 2

  parameterVariances <- diag(FisherInv)

  if (any(parameterVariances < 0)) {
    stop(
      "Negative variance estimates were obtained from the inverse ",
      "Fisher information matrix."
    )
  }

  SE <- sqrt(parameterVariances)
  names(SE) <- parameterNames

  parameterCorrelation <- cov2cor(FisherInv)
...
#' Calculate Standard Errors for Variance and Correlation Parameters
#'
#' @description
...
#'
#' @param EM3.cov.res A list returned from the \code{EM3.cov} function.
...
#' \code{"Z"} and \code{"K"}.
#'
#' @param covList A list of covariance matrices between random effects.
...
#'
#' @param check.Vinv If \code{TRUE} and \code{EM3.cov.res$Vinv} is
...
#' }
#'
#' @example R/examples/calcParamSE.cov_example.R
#'
#' @export
calcParamSE.cov <- function(
    y,
    EM3.cov.res,
    ZETA,
    covList,
  ...
  missingResults <- requiredResults[
    !requiredResults %in% names(EM3.cov.res)
  ]

  if (length(missingResults) > 0) {
    stop(
      paste0(
        "The following elements are missing from EM3.cov.res: ",
        paste(missingResults, collapse = ", "),
        "."
      )
    )
  ...
  if (length(covList) != nKernels) {
    stop(
      "The number of elements in covList should be equal to ",
      "the number of random effects in ZETA."
    )
  }
  ...
  if (!all(
    ...
    stop(
      "Each element of covList should contain nKernels elements."
    )
  }
  ...
  for (i in seq_len(nKernels)) {
    ...
    if (is.null(ZETA[[i]]$Z) ||
        is.null(ZETA[[i]]$K)) {
      stop(
        "Each element of ZETA should contain elements named 'Z' and 'K'."
      )
    }

    Zi <- as.matrix(ZETA[[i]]$Z)
    Ki <- as.matrix(ZETA[[i]]$K)
  ...
  Vu <- EM3.cov.res$Vu
  Ve <- EM3.cov.res$Ve
  weights <- EM3.cov.res$weights
  rhosMat <- as.matrix(EM3.cov.res$rhosMat)
  ...
  if (!is.numeric(Vu) ||
      ...
      Vu < 0) {
    stop(
      "EM3.cov.res$Vu should be a non-negative finite numeric value."
    )
  }

  if (Vu == 0) {
    stop(
      "EM3.cov.res$Vu is zero. Standard errors cannot be calculated ",
      "for the covariance parameterization because derivatives with ",
      "respect to kernel-specific variance components are undefined ",
      "at zero variance."
    )
  }
  ...
  if (!is.numeric(Ve) ||
      ...
      Ve <= 0) {
    stop(
      "EM3.cov.res$Ve should be a positive finite numeric value."
    )
  }

  if (!is.numeric(weights) ||
      length(weights) != nKernels ||
      any(!is.finite(weights))) {
    stop(
      "EM3.cov.res$weights should contain finite numeric values ",
      "for all kernels."
    )
  }
  ...
  if (any(weights < 0)) {
    stop(
      "All elements of EM3.cov.res$weights should be non-negative."
    )
  }
  ...
  if (!is.finite(sum(weights)) ||
      sum(weights) <= 0) {
    stop(
      "The sum of EM3.cov.res$weights should be positive and finite."
    )
  }
  ...
  if (abs(sum(weights) - 1) > 1e-8 &&
      !suppressWarning) {
    warning(
      "EM3.cov.res$weights do not sum to one. ",
      "The kernel-specific variance components are reconstructed ",
      "using the weights as returned by EM3.cov."
    )
  }

  if (!all(dim(rhosMat) == c(nKernels, nKernels))) {
    stop(
      "EM3.cov.res$rhosMat should be an nKernels x nKernels matrix."
    )
  }
  ...
  if (any(!is.finite(variances))) {
    stop(
      "Non-finite kernel-specific variance estimates were detected."
    )
  }

  if (any(variances < 0)) {
    stop(
      "Kernel-specific variance estimates (Vu * weights) should be ",
      "non-negative."
    )
  }
  ...
  if (any(variances == 0)) {
    stop(
      "Zero kernel-specific variance estimate(s) were detected. ",
      "Standard errors cannot be calculated for the covariance ",
      "parameterization because derivatives with respect to ",
      "kernel-specific variance components contain terms proportional ",
      "to 1 / sqrt(sigma_i^2)."
    )
  }
  ...
  if (any(!is.finite(rhosMat))) {
    stop("Non-finite values were found in EM3.cov.res$rhosMat.")
  }
  ...
  if (rhoDifference > sqrt(.Machine$double.eps)) {
    stop("EM3.cov.res$rhosMat should be symmetric.")
  }
  ...
  CList <- vector("list", nKernels)
  names(CList) <- kernelNames
  ...
  for (i in seq_len(nKernels)) {
    ...
    names(CList[[i]]) <- kernelNames
  }

  if (nKernels >= 2L) {

    for (i in seq_len(nKernels - 1L)) {

      for (j in (i + 1L):nKernels) {

        Kij <- covList[[i]][[j]]
        Kji <- covList[[j]][[i]]

        if (is.null(Kij) || is.null(Kji)) {
          stop(
            paste0(
              "Both covList[[",
              i,
        ...
        if (nrow(Kij) != ncol(ZList[[i]]) ||
            ncol(Kij) != ncol(ZList[[j]])) {
          stop(
            paste0(
              "The dimension of covList[[",
              i,
              "]][[",
              j,
              "]] is inconsistent with the corresponding Z matrices."
            )
          )
        }

        if (nrow(Kji) != ncol(ZList[[j]]) ||
            ncol(Kji) != ncol(ZList[[i]])) {
          stop(
            paste0(
              "The dimension of covList[[",
              j,
  ...
  if (check.Vinv &&
      !is.null(EM3.cov.res$Vinv)) {

    VinvEM3 <- as.matrix(EM3.cov.res$Vinv)

    if (all(dim(VinvEM3) == c(n, n))) {
      ...
      if (is.finite(VinvDifference) &&
        ...
        warning(
          paste0(
            "The reconstructed inverse variance-covariance matrix ",
            "differs from EM3.cov.res$Vinv (maximum absolute ",
            "difference = ",
            signif(VinvDifference, 5),
            "). This may occur when the covariance matrix used ",
            "inside EM3.cov was modified or approximated."
          )
        )
      }
      ...
      if (!suppressWarning) {
        warning(
          "EM3.cov.res$Vinv was not compared with the reconstructed ",
          "matrix because their dimensions were different."
        )
      }
    }
  ...
  VDerivVariance <- vector(
    "list",
    nKernels
  )

  names(VDerivVariance) <-
    paste0(
      "sigma2.",
      kernelNames
    )
  ...
  if (min(fisherEigen) <= fisherTol) {
    stop(
      paste0(
        ...
        "nearly singular (minimum eigenvalue = ",
        signif(min(fisherEigen), 5),
        "). Variance or correlation parameters may not be ",
