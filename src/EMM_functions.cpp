...
//   
//   if(p == pMax){
//     Fstat = beta(p - 1, 0) * beta(p - 1, 0) / CovBeta(p - 1, p - 1);
//   } else {
//     MatrixXd CovBetaPart = CovBeta.block(p - 1, p - 1, pMax - p + 1, pMax - p + 1);
//     MatrixXd CovBetaPartInv = inv(CovBetaPart);
//     MatrixXd betaPart = beta.block(p - 1, 0, pMax - p + 1, 1);
//     MatrixXd FstatMat = crossprod(betaPart, CovBetaPartInv) * betaPart / v1;
//     Fstat = FstatMat(0, 0);
//   }
//   
